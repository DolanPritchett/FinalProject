...
from commpy.channelcoding.interleavers import RandInterlv  # Fixed import
...
interleaver = RandInterlv(5000, seed=42)  # Updated to use RandInterlv
num_iterations = 100
noise_variance = 20  # Example noise variance for AWGN channel

# Generate random input sequence
msg_bits = np.random.randint(0, 2, 5000)

# Turbo encode the sequence
encoded = np.concatenate(turbo.turbo_encode(msg_bits, trellis, trellis, interleaver))  # Flatten the encoded output

# Simulate AWGN channel
received = encoded + np.random.normal(0, np.sqrt(noise_variance), len(encoded))  # Add noise

# Separate received systematic and parity bits
sys_bits = received[:len(msg_bits)]
parity1_bits = received[len(msg_bits):2*len(msg_bits)]
parity2_bits = received[2*len(msg_bits):]
#sys_bits = received[::3]
