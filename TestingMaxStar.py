...
maxStarInputPlus = np.array([ -8.06710635,  -5.15090288,  -8.51087644,  -3.18563532, -12.06103719,
  -4.58034133, -12.50480729,  -2.61507377])
maxStarInputMinus = np.array([ -6.41881743,  -3.50261396, -11.42707992,  -6.10183879,  -8.00371062,
  -0.52301475, -13.01197311,  -3.12223959])
for k in range (M*Mc):
    print("k: ", k)
    ...
    PlusAccum = maxStarInputPlus[i]

    MinusAccum = maxStarInputMinus[i]
    ...
    for i in range(1,num_sequences):
        ...
        PlusAccum = maxStar(PlusAccum, maxStarInputPlus[i])
        MinusAccum = maxStar(MinusAccum, maxStarInputMinus[i])
        #print("PlusAccum = ", PlusAccum, " = -1 * (2 * ", sigma2,"* np.linalg.norm(", Y," - ", H," @ ",SPlus[i].reshape(-1, 1),"**2 + 0.5 * ", PlusSumAccum)
